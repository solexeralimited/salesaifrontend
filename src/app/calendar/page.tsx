        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="grid grid-cols-7">
            {Array.from({ length: firstDay }).map((_, i) => (
              <div key={`empty-${i}`} className="border-b border-r border-gray-100 min-h-[100px] p-2 bg-gray-50" />
            ))}
            {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
              ...
              return (
                <div key={day} className={`border-b border-r border-gray-100 min-h-[100px] p-2 ${isToday ? 'bg-blue-50' : ''}`}>
                  <div className={`text-sm font-medium mb-1 ${isToday ? 'text-blue-700' : 'text-gray-700'}`}>{day}</div>
                  {dayMeetings.map(m => (
                    <div key={m.id} className={`text-[10px] px-1.5 py-0.5 rounded mb-0.5 truncate ${statusColor[m.status] || 'bg-blue-100 text-blue-800'}`}>
                      {new Date(m.scheduled_at).toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit' })} {m.lead_name}
                    </div>
                  ))}
                </div>
              );
          ...
          <div className="mt-6">
            ...
            <div className="space-y-2">
              {meetings
                ...
                .map(m => (
                  <div key={m.id} className="bg-white border border-gray-200 rounded-lg px-4 py-3 flex items-center gap-4">
